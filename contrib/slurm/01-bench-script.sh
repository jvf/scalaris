...
for TASKSPERNODE in `seq 2 $VMS_PER_NODE`; do
    erl -setcookie "chocolate chip cookie" -name bench_ -noinput -eval "N = rpc:call('node$PORT@`hostname -f`', comm_stats, get_no_of_ch, []), io:format('number of channels: ~w~n', [N]), halt(0)."
    let PORT+=1
done
